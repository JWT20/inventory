...
| `VAPID_PUBLIC_KEY` | Public Web Push application-server key | `` (push disabled) |
| `VAPID_PRIVATE_KEY` | Private Web Push application-server key; keep secret and stable | `` (push disabled) |
| `VAPID_SUBJECT` | HTTPS URL or `mailto:` contact used in VAPID claims | `` (push disabled) |
| `PUSH_DISPATCH_INTERVAL_SECONDS` | Interval for draining the push outbox | `5` |
| `KAFKA_BOOTSTRAP_SERVERS` | Kafka broker address (optional) | `kafka:9092` |
| `LANGFUSE_PUBLIC_KEY` | Langfuse public key (optional) | `` |
| `LANGFUSE_SECRET_KEY` | Langfuse secret key (optional) | `` |
| `LANGFUSE_HOST` | Langfuse host URL | `https://cloud.langfuse.com` |
| `CHANNEL_CREDENTIAL_ENCRYPTION_KEY` | Base64url 32-byte key for encrypted Shopify OAuth tokens | generated by `deploy/create-env.sh` |
| `ADVICE_STOCK_API_KEY` | Secret for the read-only wine advice stock endpoint | `` (integration disabled) |
| `ADVICE_SALES_API_KEY` | Secret for advice-app store writes (counter sales + pickup reservations); revocable on its own | `` (integration disabled) |
...
Generate a VAPID key pair once, copy both values into the production `.env`,
and keep that pair across deployments:

```bash
python backend/scripts/generate_vapid_keys.py
```

Owners, members, and couriers can then opt in per device with the bell in the
app header. Customer-created orders notify that organization's merchants;
pickable approved orders and new live EAN channel orders notify the courier.
...
| Method | Path | Auth | Description |
|--------|------|------|-------------|
| GET | `/api/push/config` | Merchant/Courier | Push availability and public VAPID key |
| POST | `/api/push/subscriptions` | Merchant/Courier | Enable or refresh this browser subscription |
| DELETE | `/api/push/subscriptions` | Merchant/Courier | Disable this browser subscription |
...
#    POSTGRES_PASSWORD, SECRET_KEY and CHANNEL_CREDENTIAL_ENCRYPTION_KEY with
#    openssl when they are not supplied. When restoring a database backup,
#    supply the original CHANNEL_CREDENTIAL_ENCRYPTION_KEY instead.
...
ADVICE_PRODUCTS_API_KEY=<separate-strong-shared-secret>
ADVICE_PRODUCTS_SYNC_INTERVAL_SECONDS=3600
```

Het volledige contract en de veiligheidsregels staan in
[`docs/advice-product-integration.md`](docs/advice-product-integration.md).
